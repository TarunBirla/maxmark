...
.logo{
  display:flex; flex-direction:column; color:var(--paper-2);
}
...
<header class="site-header" id="siteHeader">
  <div class="wrap">
    <a href="#home" class="logo">
      <span class="word">MAX<span>MARK</span></span>
      <span class="sub">BUILDERS · LONDON</span>
    </a>
    <nav class="main-nav">
      <ul>
      ...
      <div class="quote-step" data-step="4">
        <div class="quote-result">
          ...
          <div class="contact-fields">
            <div class="field"><label>FIRST NAME</label><input type="text" placeholder="Jane" required></div>
            <div class="field"><label>PHONE</label><input type="tel" placeholder="07xxx xxxxxx" required></div>
            <div class="field full"><label>EMAIL</label><input type="email" placeholder="you@email.com" required></div>
            <div class="field full"><label>POSTCODE</label><input type="text" placeholder="e.g. TW3 1AB"></div>
          </div>
          <div class="quote-nav" style="justify-content:space-between;">
            <button class="btn btn-outline" onclick="prevStep()">← Back</button>
            <button class="btn btn-signal" onclick="submitQuote()">Send me my exact quote <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16"><path d="M5 12h14M13 6l6 6-6 6"/></svg></button>
          </div>
        </div>
      </div>
  ...
  <div class="wrap">
    <div class="footer-grid">
      <div class="footer-brand">
        <span class="word" style="font-family:'Space Grotesk';font-size:22px;color:var(--paper-2);font-weight:700;">MAX<span style="color:var(--brass-light);">MARK</span></span>
...
function submitQuote() {
  const inputs = document.querySelectorAll('#quote .contact-fields input');
  let valid = true;
  inputs.forEach(inp => { if (inp.required && !inp.value.trim()) { inp.style.borderColor = '#e4572e'; valid = false; } });
  if (!valid) return;
  goToStep(5);
}
